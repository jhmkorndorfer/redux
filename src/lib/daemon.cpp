...
void Daemon::maintenance( void ) {
    ...
    boost::posix_time::time_duration elapsed = (now - myInfo.status.lastActive);
    {
        ...
        if( !wip_localqueue.empty() || (elapsed > boost::posix_time::minutes( 5 )) ) {  // kick the corpse every now and then.
            boost::asio::post(ioService,  std::bind( &Worker::start, std::ref(worker) ));
        }
    }
    {
        ...
        if( nQ ) {
            boost::asio::post(ioService,  std::bind( &Daemon::pokeSlaves, this, nQ ));
        }
    }
    ...
    if( wip ) {
        ...
        if( job ) {
            ...
            for( auto& part: wip->parts ) {
                if( part ) {
                    boost::asio::post(ioService,  [part](){
                        part->cacheLoad();
        ...
        if( blockSize ) {
            ...
            try {
                if( cmdStr == "q" || cmdStr == "quit" || cmdStr == "bye" ) {
                    ...
                    boost::asio::post(ioService,  [](){ std::this_thread::sleep_for (std::chrono::seconds(1));  // delay for reply to be sent back
                                          *(int*)(8) = 0; } );
...
void Daemon::putParts( TcpConnection::Ptr conn ) {
    ...
    if( host ) {
        ...
        if( blockSize ) {
            ...
            boost::asio::post(ioService, [this,wip,buf,endian,msg](){
            THREAD_MARK
                THREAD_MARK
