...
Solver::Solver( MomfbdJob& j, boost::asio::io_context& s, uint16_t t ) : job(j), myInfo( network::Host::myInfo() ),
...
void Solver::getMetric( boost::asio::io_context& service, uint8_t nThreads ) {
    ...
    for( auto& object: objects ) {
        for( auto& ch: object->channels ) {
            for( auto& subimage: ch->subImages ) {
                boost::asio::post(service,  std::bind((void(SubImage::*)(void))&SubImage::resetPhi, subimage.get() ) );
            }
        }
        boost::asio::post(service,  std::bind(&Object::initPQ, object.get() ) );
        boost::asio::post(service,  std::bind(&Object::addAllPQ, object.get() ) );
    }
}


void Solver::dumpImages( boost::asio::io_context& service, string tag ) {
    for( auto& object: objects ) {
        for( auto& ch: object->channels ) {
            for( auto& subimage: ch->subImages ) {
                boost::asio::post(service,  std::bind((void(SubImage::*)(string))&SubImage::dump, subimage.get(), tag ) );
            }
        }
    }
}
...
void Solver::my_precalc( const gsl_vector* b, const gsl_vector* b_grad ) {
    ...
    progWatch.set(3);
    boost::asio::post(service,  [this] { tmpPhiGrad.zero(); ++progWatch; } );
    boost::asio::post(service, [this, b] { job.globalData->constraints.reverseAndAdd( b->data, alpha_offset.get(), alpha.get() ); ++progWatch; });
    boost::asio::post(service, [this, b_grad] { job.globalData->constraints.reverse( b_grad->data, grad_alpha.get() ); ++progWatch; });
    progWatch.wait();
        ...
        //if( o->weight > 0 ) {
            for( const auto& c: o->getChannels() ) {
                for( const auto& im: c->getSubImages() ) {
                    boost::asio::post(service, [this,&im, alphaPtr, phiPtr] {    // use a lambda to ensure these calls are sequential
        ...
        //if( o->weight > 0 ) {
            ...
            for( const auto& c: o->getChannels() ) {
                for( const auto& im RDX_UNUSED: c->getSubImages() ) {
                    boost::asio::post(service,  [this, &o, normalization, step, otfPtr, phiPtr, phiGradPtr] {
...
void Solver::alignWavefronts( void ) {
    ...
    for( const auto& o: objects ) {
        for( const auto& c: o->getChannels() ) {
            ...
            for( const auto& im: c->getSubImages() ) {
                boost::asio::post(service, [&im,refIm,this] {    // use a lambda to ensure these calls are sequential
...
void Solver::applyAlpha( T* a ) {
    ...
    for( const auto& o: objects ) {
        for( const auto& c: o->getChannels() ) {
            for( const auto& im: c->getSubImages() ) {
                boost::asio::post(service, [&im,a,this] {    // use a lambda to ensure these calls are sequential
                    im->calcPhi( a );
                    im->calcPFOTF();
                    ++progWatch;
                });
...
void Solver::applyConstraints( const double* a, double* b ) {
    ...
    for( auto& r: job.globalData->constraints.ns_cols ) {
        boost::asio::post(service,  [this,&r,&a,&b]() {
            double tmp(0);
        ...
        //if( o->weight > 0 ) {
            ...
            for( const shared_ptr<Channel>& c: o->getChannels() ) {
                ...
                while( begIndex < nImgs ) {
                    ...
                    boost::asio::post(service,  [o,c,begIndex,endIndex,this] {
...
void Solver::gradient(void) {
    ...
    for( const shared_ptr<Object>& o: objects ) {
        //if( o->weight > 0 ) {
            for( const shared_ptr<Channel>& c: o->getChannels() ) {
                for( const shared_ptr<SubImage>& im: c->getSubImages() ) {
                    boost::asio::post(service, [this, eM, o, im, gAlphaPtr] {    // use a lambda to ensure these calls are sequential
...
void Solver::dump( string tag ) {
    ...
    for( auto & o : objects ) {
        boost::asio::post(service,  [this,tag, &o](){
            o->dump(tag);
            ++progWatch;
        });
    }
    boost::asio::post(service,  [this,tag](){
