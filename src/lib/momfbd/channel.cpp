...
void Channel::loadCalib( boost::asio::io_context& service ) {     // load through cache-functionality, so the same data is recycled for other objects
...
void Channel::loadData( boost::asio::io_context& service, redux::util::Array<PatchData::Ptr>& patches ) {
    ...
    for( unsigned int y=0; y<patches.dimSize(0); ++y ) {
        for( unsigned int x=0; x<patches.dimSize(1); ++x ) {
            boost::asio::post(service,  [this,&patches,y,x]() {
    ...
    for( size_t i=0; i<nFiles; ++i ) {
        boost::asio::post(service,  [&,i,nPreviousFrames,saveFFData](){
            try {
                ...
                for( size_t j=0; j<nF; ++j ) {
                    ...
                    //boost::asio::post(service, std::bind( &Channel::preprocessImage, this, nPreviousFrames+j) );
                }
                if( saveFFData ) {
                    bfs::path fn;
                    try {
